function processData(midiData) {
	...
	for (var i = 0; i < midiData.tracks; i++) {
		...
		for (var j = 0; j < midiData.track[i].event.length; j++) {
			...
			if (event.type == 9) { //note on events
				...
				if (index != -1) {
					events.push({ type: "noteon", tick: currentTick, pitch: index, note: notes[event.data[0]], channel: event.channel, track: i, velocity: event.data[1] });
				}
			}
	...
	var minimumDistanceBetweenNotes = 1;
